...
$N = \sum_{i=1}^N \det A_i = \sum_{i=1}^N (a_i d_i - b_i c_i)$

\textbf{3.1.} Drawing based on Fractal geometry

\textbf{Deterministic Algorithm}

Let, be an IFS,

Compute directly a sequence of sets,

starting from an initial set $A \subseteq \mathbb{R}^2$

$A_{n+1} = \bigcup_{j=1}^N w_j(A_n)$, $n = 1, 2, \ldots$
...
Let be an IFS, where has been $1, 2, \ldots, N$ assigned to $i$, $\sum p_i = 1$

Choose $x \in X$ and then choose recursively, independently,

$x_n \in \{ w_1(x_{n-1}), w_2(x_{n-1}), \ldots, w_N(x_{n-1}) \}$, $n = 1, 2, 3, \ldots$

Where the probability of the event $x_n = w_i(x_{n-1})$ is $p_i$

Sequence $\{x_n\}$ be constructed converges to the attractor of
...
Based on theorem of contraction mapping sequence in
